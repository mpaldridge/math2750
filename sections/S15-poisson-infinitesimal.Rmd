...
Since the increments as defined in \@ref(eq:tau) are stationary, it will suffice to consider $s = 0$. So we need to show that $X(t) \sim \text{Po}(\lambda t)$. 
...
\end{align*}
These are often referred to as the **Kolmogorov forward equations**.
...
This has $p_0(0) = \ee^{0} = 1$, and we have
\[ p'_0(t) = -\lambda \ee^{-\lambda t} = -\lambda p_0(t) , \]
as desired.

For $j \geq 1$, we have $p_j(0) = \ee^{0} 0^j/j! = 0$, and the left-hand side is
\begin{align*}
