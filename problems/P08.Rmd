...
::: {.myanswers data-latex=""}
*Solution.* The waiting time until the next arrival is Exp$(\lambda)$ with mean $1/\lambda$. Working backwards, time since last arrival EXPLAIN EXPLAIN is also Exp$(\lambda)$. Total is $2/\lambda$.
:::
::::
...
::: {.myanswers data-latex=""}
*Solution.* Eg. the time $t$ is more likely to have landed in a longer-than-average wait time, because there's more space to land in.
:::
::::
:::::
...
::: {.myanswers data-latex=""}
*Solution.* 
	For $j = 1$, we have $p_1(t) = \ee^{-\lambda t}$. This clearly has $p_1(0) = \ee^0 = 1$, and $p'(j) = -\lambda \ee^{-\lambda t}$, as required.
...
:::: {.subq}
**(b)** Show that, as $n \to \infty$, the expectation $\mathbb E J_n$ tends to infinity, but the variance $\Var(T_n)$ is bounded.
