...
::: {.myanswers data-latex=""}
*Solution.* You could answer this question by using what you know about maximum likelihood estimation for the binomial distribution from MATH1712 or MATH2715, but we'll reason from first principles here.
  
If we let $k = (n + x_n)/2$ be the number of upward steps. Then the "likelihood" is the probability mass function
  \[ f(\mathbf x; p) = p^{k}(1-p)^{n-k} , \]
since we take $k$ steps up and $n-k$ steps down.
As is often the case, it's more convenient to use the log-likelihood
  \[ \ell(\mathbf x; p) = \ln f(\mathbf x; p) = k \ln p + (n-k)\ln(1-p) .\]
Given $\mathbf x$ (and therefore given $k$), we find the maximum likelihood estimate by finding the $p$ that maximises this likelihood (or log-likelihood). We can perform the maximisation by differentiating and setting equal to $0$. The derivative is
