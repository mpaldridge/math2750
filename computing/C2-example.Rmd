...
<span style="color: blue;">[2 marks are available for each question, for a total of 8.]</span>
...
<span style="color: blue;">[1 mark for code that correctly produces a sample of the Markov chain.]</span>
...
<span style="color: blue;">[1 mark for a graph and/or a comment.]</span>
...
<span style="color: blue;">[1 mark for code that correctly calculates the proportion of time spent in each state.]</span>
...
<span style="color: blue;">[1 mark for recognising that for large $N$ the proportion of time in each state should be similar to stationary distribution, and making a comparison. Don't award the mark if small $N$ (eg $N = 50$) is used, leading the two quantities to be very different.]</span>
...
<span style="color: blue;">[1 mark for code that correctly calculates the mean return times.]</span>

A result from the course tells us that the expected return times should be equal to the reciprocal of the stationary distribution; that is, $\mu_i = 1/\pi_i$, where $\boldsymbol\pi = (\pi_i)$ is the stationary distribution.
...
<span style="color: blue;">[1 mark for recognising that for large $N$ the mean return times should be close to the reciprocals of the stationary distribution and marking a comparison. Don't award the mark if small $N$ (eg $N = 50$) is used, leading the two quantities to be very different.]</span>
...
<!-- Change the penultimate argument to eval = FALSE if the Tikz picture doesn't compile properly -->
...
<span style="color: blue;">[1 mark for recognising that small $\epsilon$ requires a large $N$. (I expect not that many students to get this mark.)]</span>
...
<span style="color: blue;">[1 mark for recognising that $\epsilon = 0$ splits the chain into two different parts, so the theoretical results don't apply.]</span>
