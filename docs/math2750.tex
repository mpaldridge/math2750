...
\item
  \textbf{Lecture}: Tuesday at 1400
\item
  \textbf{Drop-in sessions}: Tuesday or Wednesday
\end{itemize}
...
\tightlist
\item
  I don't understand something in the notes or on a problem sheet: Go to your weekly drop-in session, or post a question on \href{https://teams.microsoft.com/l/channel/19\%3a5fcd058b7074426ca1f7d1cf2052d3b4\%40thread.tacv2/Q\%2520and\%2520A?groupId=1c138eac-0c54-43b0-9d20-d4cf3d65c40a\&tenantId=bdeaeda8-c81d-45ce-863e-5232a535b7cb}{the Teams Q and A board}.
\item
  I don't understand something in on a computational worksheet: Go to your computing drop-in session in weeks 4 or 7.
\item
  I have an admin question about general arrangements for the module: \href{mailto:m.aldridge@leeds.ac.uk}{Email the module leader}.
\item
  I have an admin question about arrangements for my workshop: Email your workshop leader.
\item
  I need an extension on or exemption from an assessment: \href{mailto:Maths.Taught.Students@leeds.ac.uk}{Email the Maths Taught Students Office}.
\end{itemize}
...
\item
  Forwardequations for the Poisson process
\end{itemize}
...
Since the increments as defined in \eqref{eq:tau} are stationary, it will suffice to consider \(s = 0\). So we need to show that \(X(t) \sim \text{Po}(\lambda t)\).
...
\[ \frac{p_j(t + \tau) - p_j(t)}{\tau} = -\lambda p_j(t) + \lambda p_{j-1}(\tau) + \frac{o(\tau)}{\tau} . \]
...
After rearranging and sending \(\tau \to 0\), we get the differential equation
\[ p'_0(t) = -\lambda p_0(t) \]
...
\end{align*}
These are often referred to as the \textbf{Kolmogorov forward equations}.
...
This has \(p_0(0) = \mathrm{e}^{0} = 1\), and we have
\[ p'_0(t) = -\lambda \mathrm{e}^{-\lambda t} = -\lambda p_0(t) , \]
as desired.

For \(j \geq 1\), we have \(p_j(0) = \mathrm{e}^{0} 0^j/j! = 0\), and the left-hand side is
\begin{align*}
...
\end{align*}
as desired. We are finally done.
