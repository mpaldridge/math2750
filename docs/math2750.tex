...
\hypertarget{TIPP}{%
\subsection{Time inhomogeneous Poisson processes}\label{TIPP}}
...
\begin{myanswers}
\emph{Solution.} The waiting time is Exp\((\lambda)\) with mean \(1/\lambda\).
...
\begin{myanswers}
\emph{Solution.} The waiting time until the next arrival is Exp\((\lambda)\) with mean \(1/\lambda\). Working backwards, time since last arrival EXPLAIN EXPLAIN is also Exp\((\lambda)\). Total is \(2/\lambda\).
...
\begin{myanswers}
\emph{Solution.} Eg. the time \(t\) is more likely to have landed in a longer-than-average wait time, because there's more space to land in.
...
\begin{myanswers}
\emph{Solution.}
For \(j = 1\), we have \(p_1(t) = \mathrm{e}^{-\lambda t}\). This clearly has \(p_1(0) = \mathrm{e}^0 = 1\), and \(p'(j) = -\lambda \mathrm{e}^{-\lambda t}\), as required.

Now consider \(j \geq 2\). We indeed have \(p_j(0) = 0\). By the product rule, the derivative is
\begin{align*}
...
\textbf{(b)} Show that, as \(n \to \infty\), the expectation \(\mathbb E J_n\) tends to infinity, but the variance \(\operatorname{Var}(T_n)\) is bounded.
