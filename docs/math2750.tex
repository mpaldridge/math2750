...
\begin{myanswers}
\emph{Solution.} The time between the arrivals is \(S + T\), where \(S\) is the time backwards from \(t\) since the previous arrival, and \(T\) is the time forwards from \(t\) until thenext arrival. By the memoryless property of the exponential distribution, \(T \sim \operatorname{Exp}(\lambda)\), with mean \(1/\lambda\). What about \(S\)? We have
...
\begin{myanswers}
\emph{Solution.} Imagine the arrivals being placed on the real line -- some with big gaps between them, and some with small gaps between them. If we then place the time \(t\), it is more likely to be placed in one of the large gaps than one of the small ones, because each large gap fills up more of the line than a small one. Hence the size of the gap surrounding the given point \(t\) is ``size-biased'', and is larger than a unfirotmly randomly chosen gap.
...
\[ \mathbb P \big( Y(t + \tau) - Y(t) = 1\big) = p\lambda t + o (\tau) .   \]
...
\[ \mathbb P \big( Y(t + \tau) - Y(t) = 0 \big) = 1 - \lambda t + (1-p)\lambda t + o(\tau) = 1 - p\lambda t + o(\tau)  .   \]
...
\[ \mathbb P(T_1 > \tau) = \mathrm{e}^{-q_i \tau} = 1 - q_i\tau + o(\tau) , \]
...
where we interpret \(\mathsf A^0 = \mathsf I\) to be the identity matrix. In particular, since a matrix commutes with itself, we have \(\mathsf {A}^n \mathsf A = \mathsf{AA}^n\), so a matrix commutes with its own exponential, meaning that \(\mathsf A \mathrm{e}^{\mathsf A} = \mathrm{e}^{\mathsf A} \mathsf A\).
