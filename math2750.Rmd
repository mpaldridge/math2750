...
If you there is something in the course you wish to discuss in detail, the place for the is the optional weekly drop-in session. You will have been assigned to one of three groups on Tuesdays or Wednesdays with [Nikita Merkulov](https://eps.leeds.ac.uk/maths/pgr/4992/nikita-merkulov) or me. The drop-in sessions will be run the Microsoft Teams. Your drop-in session would be an excellent place to go if you are having trouble understanding something in the written notes, or if you're still truggling on a problem sheet question after your workshop.
...
![](http://www.youtube.com/watch?v=gkakqCz_M5Q)
...
<!--chapter:end:sections/S01-stochastic-processes.Rmd-->
...
```{r rw-pics, fig.cap ="Two simulations of random walks.", out.width = "50%", fig.show = "hold", fig.width = 5, echo = FALSE}
set.seed(315)
...
<!--chapter:end:sections/S02-random-walks.Rmd-->
...
```{tikz dice, echo=FALSE, fig.align="center", fig.width=4}
...
<!--chapter:end:sections/S03-gamblers-ruin.Rmd-->
...
When the right-hand side of \@ref(eq:lde) is zero, so $f(n) = 0$, we say the equation is **homogeneous**; if the right-hand side is nonzero, it is **inhomogeneous**. The number $k$, where there are $k+1$ terms on the left-hand side, is called the **degree** of the equation; we are mostly interested in second-degree linear difference equations, which have three terms on the left-hand side.
...
If you think you've got the hang of this, you should press on to [the next subsection](#S04-ruin-probability). But if you're still a bit unsure, here are two extra optional examples. (These also give an idea of how I would expect you to set out your answers to similar problems.)
...
2.  Find any solution (a "particular solution") to the inhomogeneous equation. The general solution to the inhomogeneous equation is a particular solution plus the general solution to the homogeneous equation.
3. Use the extra conditions to find the values of the constants in the general solution.

This idea works because adding a solution to the *homogeneous* equation to the left-hand side adds zero to the right-hand side, so maintains a solution to the inhomogeneous equation.
...
We now need to find a **particular solution** -- that is, any solution -- to our new inhomogeneous equation. A general hint here is to guess a solution with the same "shape" as the right-hand side. For example, if the right-hand side is a constant, try a constant for the particular solution. Here our right-hand side is the constant $2$, so we should try a constant $x_n = C$. Substituting this into the inhomogeneous equation gives us $C - 5C + 6C = 2$, thus $2C = 2$ and $C = 1$, giving a particular solution $x_n = 1$. The general solution to the inhomogeneous equation is therefore
  \[ x_n = 1 + A2^n + B3^n , \]
...
Again, if you have the hang of this, [press on](#duration-solve), but we present an extra optional example if things are still unclear.
...
::: {.mysummary}
**In the next section**, we move on to the general theory of discrete time Markov chains.
:::

<!--chapter:end:sections/S04-ldes.Rmd-->
...
:::::: {.myq}
**4.** Consider the gambler's ruin problem with draws: at each step, Alice wins £1 with probability $p$, loses £1 with probability $q$, and neither wins nor loses any money with probability $s$, where $p + q +s = 1$, and $0 < p,q,s<1$. Alice starts with £$a$ and Bob with £$(m-a)$.
...
:::: {.myq}
**5.** The Fibonacci numbers are 1, 1, 2, 3, 5, 8, 13, 21, 34, ..., where each number in the sequence is the sum of the two previous numbers. Show that the ratio of consecutive Fibonacci numbers tends to the "golden ratio" $\phi = (1 + \sqrt{5})/2$.
...
<!--chapter:end:assessments/A1.Rmd-->
...
```{tikz twostate, echo=FALSE, fig.align="center", fig.width=4, fig.cap="Transition diagram for the two-state Markov chain"}
...
<!--chapter:end:sections/S05-markov-chains.Rmd-->
...
<!--chapter:end:sections/S06-examples.Rmd-->
...
```{tikz HSD, echo=FALSE, fig.align="center", fig.width=4}
\usetikzlibrary{automata}
...
When [we discussed the simple random walk, we noted](#S02-exact-distribution) that it alternates between even-numbered and odd-numbered states. This "periodic" behaviour is important to understand if we want to know what will happen to the Markov chain in the future.
...
```{tikz periods, echo=FALSE, fig.align="center", fig.width=5}
\usetikzlibrary{automata}
...
<!--chapter:end:sections/S07-classes.Rmd-->
...
<!--chapter:end:sections/S08-hitting-times.Rmd-->
...
<!--chapter:end:sections/S09-recurrence-transience.Rmd-->
...
<!--chapter:end:sections/S10-stationary-distributions.Rmd-->
...
One thing that *could* happen over time is that the distribution $\boldsymbol\lambda \mathsf P^n$ of the Markov chain could gradually settle down towards some "equilibrium distribution". Further, perhaps that long-term equilibrium might not depend on the initial distribution, but the initial behaviour might eventually almost disappear, exhibiting a "lack of memory" of the start of the process.
...
I particularly like how this one theorem gathers together all the ideas from the course in one result (Markov chains, irreducibility, periodicity, recurrence/transience, positive/null recurrence, return times, stationary distributions...).
...
We give an [full proof of the limit theorem](#S11-proofs) below (optional and nonexaminable). However, this result gets part way there and is straightforward.
...
::: {.example #conv1}
[The two-state "broken printer" Markov chain](#S05-example) is irreducible, aperiodic, and positive recurrent, so its stationary distribution is also the equilibrium distribution. We proved this from first principles in [Question 1 on Problem Sheet 3](#P03).
:::
...
```{tikz stat-ex, echo=FALSE, fig.align="center", fig.width=5}
\usetikzlibrary{automata}
			\begin{tikzpicture}[scale=0.75]
...
<!--chapter:end:sections/S11-long-term-chains.Rmd-->
...
<!--chapter:end:sections/S12-revision-i.Rmd-->
...
<!--chapter:end:sections/S13-poisson-poisson.Rmd-->
...
The proof of this is an exercise on [Problem Sheet 7](#P07).
...
When modelling a process $(X(t))$ counting many arrivals at rate $\lambda$, we might model the process like this: after waiting an $\Exp(\lambda)$ amount of time, an arrival appears. After another $\Exp(\lambda)$ amount of time, another arrival appears. And so on.
...
Let $(X(t))$ be a Poisson process with rate $\lambda$, as defined by its independent Poisson increments (Definition **REF REF REF**). Then $(X(t))$ has the exponential holding times structure described above.
:::
...
<!--chapter:end:sections/S14-poisson-exponential.Rmd-->

# Poisson process in infinitesimal time periods  {#S15-poisson-exponential}
...
On [Problem Sheet 7](#P07), Question 3, you proved this from the Poisson increments definition. But it's perhaps easier using the infinitesimals.
...
<!--chapter:end:sections/S15-poisson-infinitesimal.Rmd-->
